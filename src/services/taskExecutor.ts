  ...
  async executeTask(task: TodoTask): Promise<TaskExecutionResult> {
    try {
      ...
      const systemPrompt =
        ...
        `あなたはユーザーのTodoタスクを実行するAIエージェントです。
利用可能なツール（Gmail送信、Slack通知、カレンダー操作など）を積極的に活用して、タスクを実際に実行してください。

## 重要な指示
- **ツールを必ず使う**: メール、通知、カレンダー操作などのタスクは、対応するツールを呼び出して実際に実行してください
- **実行結果を報告**: ツール実行後、何をしたかを簡潔に報告してください
- **実行不可の場合**: ツールが見つからない場合のみ、「実行できませんでした」と短く報告してください（長い説明は不要）
