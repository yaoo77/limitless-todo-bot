...
## 実行可能性チェック（重要）
以下のチェックをすべて満たすタスクのみを抽出してください：
- **誰に**: 対象者が明確か（例: 田中さんに、チームに）
- **何を**: 実行内容が具体的か（例: メールを送る、資料を作成する）
- **文脈十分性**: 会話から十分な情報が得られているか

以下のような曖昧なタスクは抽出しないでください：
- 「お知らせする」「通知する」のみで内容が不明
- 「調べる」「確認する」のみで対象が不明
- 「〜してほしい」のみで具体的な行動が不明

例:
OK: 田中さんに明日の会議のリマインドメールを送る
OK: AIエージェントフレームワークmastraの情報を共有する
NG: 1ヶ月前にお知らせする（何を誰に知らせるか不明）
NG: 情報を共有する（何の情報か不明）
NG: 調べておく（何を調べるか不明）
